...
#ifndef CONNECT4_AI_AGENT_MEDIUMHEURSITIC_H
#define CONNECT4_AI_AGENT_MEDIUMHEURSITIC_H

#include "HeuristicBase.h"
#include <algorithm>

class MediumHeuristic : public HeuristicBase {
public:
    int evaluate(const Board& board, int player) override {
        ...
        // Evaluar diagonales
        evaluateDiagonals(board, player, opponent, score);
...
private:
    ...
    void evaluateDiagonals(const Board& board, int player, int opponent, int& score) {
        // Diagonal positiva
        for (int row = 0; row < Board::ROWS - 3; row++) {
            for (int col = 0; col < Board::COLS - 3; col++) {
                std::vector<int> window = {
